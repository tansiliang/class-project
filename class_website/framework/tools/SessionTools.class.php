...
class SessionTools{
    ...
    public function sess_read($sess_id) {
		$sql = "select sess_data from `gj_session` where sess_name='$sess_id'";
    ...
    public function sess_write($sess_id, $sess_data) {
		$expire = time();
		$sql = "insert into `gj_session` values ('$sess_id', '$sess_data', $expire) on duplicate key update sess_data='$sess_data', expact=$expire";
    ...
    public function sess_destroy($sess_id) {
		$sql = "delete from `gj_session` where sess_name='$sess_id'";
		return $this->db->query($sql);
	}
    /**
     * 在session_static()时执行
    ...
    public function sess_gc($ttl) {
		$last_time = time()-$ttl;
		$sql = "delete from `gj_session` where expact < $last_time";
		return $this->db->query($sql);
	}
